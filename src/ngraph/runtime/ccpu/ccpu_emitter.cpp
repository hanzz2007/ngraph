...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Add>(
...
static void emitBatchDot(const ngraph::Node* node,
                         ...
                         const Shape& shape_b,
                         const Shape& shape_c,
                         const std::vector<runtime::ccpu::TensorViewWrapper>& args,
                         const std::vector<runtime::ccpu::TensorViewWrapper>& out,
                         codegen::CodeWriter& writer)
{
    writer.block_begin();
    ...
    const std::vector<TensorViewWrapper>& out)
{
    ...
    if (axes.size() == 1)
    {
        ...
        {
            writer << emit_constant_array(
                out[0].get_element_type().c_type_string(), "ones", "1.0f", arg2_shape.at(0));
            ;
            emitCblasSgemmBatch(writer,
        ...
        {
            writer << emit_constant_array(
                out[0].get_element_type().c_type_string(), "ones", "1.0f", arg2_shape.at(1));
            emitCblasSgemmBatch(writer,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::BatchDot>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    emitBatchDot<ngraph::op::BatchDot>(
        node, cg->get_a_shape(), cg->get_b_shape(), out[0].get_shape(), args, out, writer);
}

template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Lstm>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Rnn>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
                                                       ...
                                                       bool append_relu)
{
    ...
    writer << "memcpy(&bn_weights[0]+" << args[0].get_size() << ", " << args[1].get_name() << ", "
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::BatchNorm>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    if (!mkldnn_utils::use_mkldnn_kernel(node))
    {
        ...
        if (batchnorm->get_training_flag() && args.size() == 3)
        {
            writer << "reference::batch_norm_three_outputs(" << batchnorm->get_eps_value() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::BatchNormRelu>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::BatchNormBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "std::vector<" << args[0].get_element_type().c_type_string() << ">bn_weights(2*"
           << args[0].get_size() << ");\n";
    writer << "std::vector<" << args[0].get_element_type().c_type_string() << ">bn_dweights(2*"
           << args[0].get_size() << ");\n";
    ...
    auto batchnorm_index = mkldnn_emitter->build_batchnorm_backward(weights_desc,
                                                                    input_desc,
                                                                    mean_desc,
                                                                    variance_desc,
                                                                    delta_desc,
    ...
    writer << "memcpy(" << out[1].get_name() << ", &bn_dweights[0], "
           << args[0].get_size() * args[0].get_element_type().size() << ");\n";
    writer << "memcpy(" << out[2].get_name() << ", &bn_dweights[0]+" << args[0].get_size() << ", "
           << args[1].get_size() * args[1].get_element_type().size() << ");\n";
    writer.block_end();
}

template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Dot>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
             ...
             dot->get_reduction_axes_count() == 1)
    {
        ...
        if (args[0].get_element_type() == element::f32)
        {
            ...
            writer << "cblas::cblas_sgemm("
                   ...
                   << "cblas::Transpose::None, " << arg0_shape[0] << ", " << arg1_shape[1] << ", "
                   << arg0_shape[1] << ",\n"
                   << "        1.0f, " << args[0].get_name() << ", " << max(1UL, arg0_shape[1])
                   << ", " << args[1].get_name() << ", " << max(1UL, arg1_shape[1]) << ", 0.0f,\n"
    ...
    // each of the
             ...
             dot->get_reduction_axes_count() == 1 && args[0].get_element_type() == element::f32)
    {
        auto mat_a = args[0];
        auto mat_b = args[1];
        auto mat_c = out[0];
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Multiply>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "for (size_t i = 0; i < " << out[0].get_size() << "; i++)\n";
    writer.block_begin();
    writer << out[0].get_name() << "[i] = " << args[0].get_name() << "[i] * " << args[1].get_name()
           << "[i];\n";
    writer.block_end();
    writer.block_end();
}
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::GetOutputElement>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Abs>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "for (size_t i = 0; i < " << out[0].get_size() << "; i++)\n";
    writer.block_begin();
    writer << out[0].get_name() << "[i] = " << (result_element_type.is_signed() ? "std::abs" : "")
           << "(" << args[0].get_name() << "[i]);\n";
    writer.block_end();
    writer.block_end();
}
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Concat>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    if (runtime::ccpu::mkldnn_utils::use_mkldnn_kernel(node))
    {
        ...
        concat_index = mkldnn_emitter->build_concat(inputs_data_desc, result_desc, concat_dim);
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Divide>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Equal>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Greater>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::GreaterEq>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Less>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::LessEq>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::LRN>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        auto& mkldnn_emitter = external_function->get_mkldnn_emitter();
        auto input_data_desc = runtime::ccpu::mkldnn_utils::get_input_mkldnn_md(node, 0);
        auto result_desc = runtime::ccpu::mkldnn_utils::get_output_mkldnn_md(node, 0);

        auto lrn_index = mkldnn_emitter->build_lrn_forward(input_data_desc,
                                                           result_desc,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Log>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Maximum>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer.block_begin();
    writer << out[0].get_name() << "[i] = " << args[0].get_name() << "[i] > " << args[1].get_name()
           << "[i] ? " << args[0].get_name() << "[i] : " << args[1].get_name() << "[i] ;\n";
    writer.block_end();
    writer.block_end();
}

template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Minimum>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Negative>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::NotEqual>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Select>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Subtract>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Broadcast>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Convert>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Constant>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Reshape>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::FunctionCall>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Reduce>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sign>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Slice>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sum>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Exp>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sin>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sinh>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Cos>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Cosh>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Tan>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Tanh>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Asin>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Acos>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Atan>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
static void emitArgMinArgMax(const std::vector<runtime::ccpu::TensorViewWrapper>& args,
                             const std::vector<runtime::ccpu::TensorViewWrapper>& out,
                             size_t reduction_axis,
                             const char* kernel_name,
                             codegen::CodeWriter& writer)
{
    if (out[0].get_element_type() != element::i64 && out[0].get_element_type() != element::i32)
    {
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ArgMin>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ArgMax>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Power>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ReplaceSlice>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::OneHot>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::one_hot<" << out[0].get_type() << ">(" << args[0].get_name() << ",\n";
        writer << "                   " << out[0].get_name() << ",\n";
        writer << "                   {" << join(args[0].get_shape()) << "},\n";
        writer << "                   {" << join(out[0].get_shape()) << "},\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Ceiling>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Floor>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sqrt>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionRelu>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::GroupConvolution>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    if (runtime::ccpu::mkldnn_utils::use_mkldnn_kernel(node))
    {
        ...
        auto weights_optimized_format = mkldnn_emitter->query_convolution_forward_weight_format(
            input_data_desc,
            weights_desc_any,
            result_desc,
            filter_strides,
        ...
        auto input_reorder_desc = mkldnn_emitter->build_memory_descriptor(
            weights_shape_groups, args[1].get_element_type(), mkldnn::memory::format::goihw);
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Convolution>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::convolution<" << out[0].get_type() << ">(" << args[0].get_name()
               << ",\n";
        writer << "                         " << args[1].get_name() << ",\n";
        writer << "                         " << out[0].get_name() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionBackpropFilters>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    if (runtime::ccpu::mkldnn_utils::use_mkldnn_kernel(node))
    {
        auto& mkldnn_emitter = external_function->get_mkldnn_emitter();
        auto conv_index =
            mkldnn_emitter->build_convolution_backward<ngraph::op::ConvolutionBackpropFilters>(
                node, args, out);
        auto& deps = mkldnn_emitter->get_primitive_deps(conv_index);
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionBackpropData>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    if (runtime::ccpu::mkldnn_utils::use_mkldnn_kernel(node))
    {
        auto& mkldnn_emitter = external_function->get_mkldnn_emitter();
        auto conv_index =
            mkldnn_emitter->build_convolution_backward<ngraph::op::ConvolutionBackpropData>(
                node, args, out);
        auto& deps = mkldnn_emitter->get_primitive_deps(conv_index);
    ...
    else
    {
        ...
        writer << "reference::convolution<" << out[0].get_type() << ">(" << args[1].get_name()
               << ",\n";
        writer << "                         " << args[0].get_name() << ",\n";
        writer << "                         " << out[0].get_name() << ",\n";
        writer << "                         {" << join(arg1_shape) << "},\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionBias>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionBiasAdd>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ConvolutionBiasBackpropFiltersBias>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    if (mkldnn_utils::use_mkldnn_kernel(node))
    {
        ...
        auto conv_index =
            mkldnn_emitter
                ->build_convolution_backward<ngraph::op::ConvolutionBiasBackpropFiltersBias>(
                    node, args, out);
        auto& deps = mkldnn_emitter->get_primitive_deps(conv_index);
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Not>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::MaxPool>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::max_pool<" << out[0].get_type() << ">(" << args[0].get_name()
               << ",\n";
        writer << "                 " << out[0].get_name() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::MaxPoolWithIndices>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Reverse>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ReverseSequence>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ReduceWindow>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "reference::reduce_window<" << out[0].get_type() << ">(" << args[0].get_name()
           << ",\n";
    writer << "                      " << args[1].get_name() << ",\n";
    writer << "                      " << out[0].get_name() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::SelectAndScatter>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "reference::select_and_scatter<" << out[0].get_type() << ">(" << args[0].get_name()
           << ",\n";
    writer << "                " << args[1].get_name() << ",\n";
    writer << "                " << args[2].get_name() << ",\n";
    writer << "                " << out[0].get_name() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::AvgPool>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::avg_pool<" << out[0].get_type() << ">(" << args[0].get_name()
               << ",\n";
        writer << "                 " << out[0].get_name() << ",\n";
        writer << "                 {" << join(arg_shape) << "},\n";
        writer << "                 {" << join(result_shape) << "},\n";
        ...
        writer << "                 " << ngraph::to_cplusplus_sourcecode_literal(
                                             avg_pool->get_include_padding_in_avg_computation())
               << "\n";
        writer << "                  );\n";
    }
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Pad>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::AvgPoolBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::avg_pool_backprop<" << out[0].get_type() << ">(" << args[0].get_name()
               << ",\n";
        writer << "                 " << out[0].get_name() << ",\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::MaxPoolBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    {
        writer << "reference::max_pool_backprop<" << out[0].get_type() << ">(" << args[0].get_name()
               << ",\n";
        writer << "                 " << args[1].get_name() << ",\n";
        writer << "                 " << out[0].get_name() << ",\n";
        writer << "                 {" << join(delta_shape) << "},\n";
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::MaxPoolWithIndicesBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Product>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Max>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Min>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::runtime::ccpu::op::ConvertLayout>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::ReluBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Relu>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::BoundedRelu>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    else
    {
        ...
        writer << args[0].get_name() << "[i] = " << args[0].get_name() << "[i] > 0 ? "
               << args[0].get_name() << "[i] : 0;\n";
        writer << out[0].get_name() << "[i] = " << args[0].get_name() << "[i] < " << alpha << " ? "
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Sigmoid>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    auto& mkldnn_emitter = external_function->get_mkldnn_emitter();
    auto input_desc =
        mkldnn::memory::desc({input_1d_size},
                             mkldnn_utils::get_mkldnn_data_type(args[0].get_element_type()),
                             mkldnn::memory::format::x);
    auto result_desc =
        mkldnn::memory::desc({result_1d_size},
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::SigmoidBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    auto input_desc =
        mkldnn::memory::desc({input_1d_size},
                             ...
                             mkldnn::memory::format::x);
    auto delta_desc =
        mkldnn::memory::desc({delta_1d_size},
                             mkldnn_utils::get_mkldnn_data_type(args[1].get_element_type()),
                             mkldnn::memory::format::x);
    auto result_desc =
        mkldnn::memory::desc({result_1d_size},
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::SigmoidMultiply>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    std::string input_0_func_string = generate_sigmoid_mul_func(
        sigmoid_mul->get_input_func_type(0), args[0].get_name() + "[i]", numer_0, denom_0, false);
    std::string input_1_func_string = generate_sigmoid_mul_func(
        sigmoid_mul->get_input_func_type(1), args[1].get_name() + "[i]", numer_1, denom_1, false);
    ...
    writer.block_end();
    writer << out[0].get_name()
           << "[i] = (" + numer_0 + " * " + numer_1 + ") / (" + denom_0 + " * " + denom_1 + ");\n";
    writer.block_end();
    writer.block_end();
}

template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::SigmoidMultiplyBackprop>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << input_0_delta.get_name()
           << "[i] = " + delta.get_name() + "[i]*(" + numer_1 + "*" + d_numer_0 + ")/(" + denom_1 +
                  "*" + d_denom_0 + ");\n";
    writer << input_1_delta.get_name()
           << "[i] = " + delta.get_name() + "[i]*(" + numer_0 + "*" + d_numer_1 + ")/(" + denom_0 +
                  "*" + d_denom_1 + ");\n";
    writer.block_end();
    writer.block_end();
}
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Softmax>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    else
    {
        ...
        else
        {
            ...
            writer << type << " (*arg)" << shape1toN << " = (" << type << " (*)" << shape1toN
                   << ") " << args[0].get_name() << ";\n";
            writer << type << " (*out)" << shape1toN << " = (" << type << " (*)" << shape1toN
                   << ") " << out[0].get_name() << ";\n";
        }
        ...
        for (size_t d = 0; d < dims; ++d)
        {
            ...
            {
                writer << "#pragma omp parallel for\n";
                writer << "for (size_t i" << d << " = 0; i" << d << " < " << shape[d] << "; ++i"
                       << d << ")\n";
                writer.block_begin();
            }
        }
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Result>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::And>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Or>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::op::Custom>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    ...
    writer << "ngraph::runtime::Backend* backend = reinterpret_cast<ngraph::runtime::Backend*>("
...
{
    auto abse = std::bind(emit_function_call, std::string("std::abs"), std::placeholders::_1);
    auto mine = std::bind(emit_function_call, std::string("std::min"), std::placeholders::_1);
    auto maxe = std::bind(emit_function_call, std::string("std::max"), std::placeholders::_1);
    auto adde = std::bind(emit_infix_operator, std::string("+"), std::placeholders::_1);
    auto nege = std::bind(emit_prefix_operator, std::string("-"), std::placeholders::_1);
...
static const ngraph::descriptor::Output* get_goe_input_output(ngraph::descriptor::Output* output)
{
    auto it = output;
    while (auto goe = std::dynamic_pointer_cast<ngraph::op::GetOutputElement>(it->get_node()))
    {
...
template <>
void ngraph::runtime::ccpu::CCPUEmitter::emit<ngraph::runtime::ccpu::op::LoopKernel>(
    CCPUExternalFunction* external_function,
    codegen::CodeWriter& writer,
    const ngraph::Node* node,
    const std::vector<TensorViewWrapper>& args,
    const std::vector<TensorViewWrapper>& out)
{
    std::unordered_map<const ngraph::descriptor::Output*, std::string> loop_symbol_table;
    ...
    for (size_t i = 0; i < node_list.size(); i++)
    {
        ...
        for (auto& input : op_node->get_inputs())
        {
            ...
            sargs.push_back(loop_symbol_table.at(get_goe_input_output(&input.get_output())));
        }

        if (std::dynamic_pointer_cast<ngraph::op::Relu>(op_node))
        {
            auto casted_zero = std::string("static_cast<") +
                               op->get_element_type().c_type_string() + std::string(">(0)");
