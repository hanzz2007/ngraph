...
struct ngraph::runtime::ccpu::OpAttributes
{
...
class ngraph::runtime::ccpu::CCPUExternalFunction
    : public std::enable_shared_from_this<CCPUExternalFunction>
{
...
public:
    ...
    const std::vector<size_t>& get_memory_buffer_sizes() const { return m_memory_buffer_sizes; }
    const std::vector<OpAttributes>& get_op_attrs() const { return m_op_attrs; }
    const std::unique_ptr<MKLDNNEmitter>& get_mkldnn_emitter() const { return m_mkldnn_emitter; }
    ...
    std::list<std::function<void(CCPURuntimeContext*)>>& get_functors() { return functors; }
    ...
    std::unordered_map<std::string, std::shared_ptr<CCPUExternalFunction>>& get_callees()
    {
        return callees;
    }
...
private:
    ...
    bool is_functionally_identical(const Node&,
                                   const Node&,
    ...
    std::function<void(CCPURuntimeContext*, std::vector<void*>&, std::vector<void*>&)> executor;
    ...
    std::list<std::tuple<std::reference_wrapper<void*>, size_t, std::reference_wrapper<bool>>>
