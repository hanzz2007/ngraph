...
function run() {
    ...
    then 
        docker run -h "$(hostname)" --privileged --name "${DOCKER_CONTAINER}" -v ${HOME}/NGRAPH_CI:/home -v "${REPO_ROOT}":/root -d ngraph-onnx:ubuntu-16_04 tail -f /dev/null
        docker exec "${DOCKER_CONTAINER}" /root/${CI_ROOT}/prepare_environment.sh
    fi
